...
class Noise(QtCore.QObject):
    ...
    def preview_noise_oneImage(self):
        ...
        for j in range(steps):
            img_noise = self.change_noise_oneImage(pil_image, value)
            pil_imagelist_noise.append([img_noise,
                                        item_name.text(),
                                        value])
            
            value = round(value + factor, 2)
            
        return pil_imagelist_noise
            
    def change_noise_oneImage(self,pil_image, factor):
